...
import LexiconClient from "./lexicon-client";

export default async function LexiconPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <main className="app-shell" data-theme={theme.preset || undefined} style={themeVars}>
      <header className="topbar">
        <div>
          ...
          <h1>Lexicon</h1>
          <p className="muted">Dictionary of in-world terms, translations, and a phoneme-based name generator.</p>
        </div>
      </header>
      <LexiconClient campaignId={campaign.id} />
    </main>
  );
}
