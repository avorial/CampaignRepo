...
async function listTemplates(token: string, campaign: NonNullable<ReturnType<typeof getCampaign>>) {
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ templates: await listTemplates(user.githubToken, campaign) });
}
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const frontmatter = {
    ...defaultFrontmatter(input.name, input.category, "gm"),
    summary: input.summary || "",
    tags: input.tags.length ? input.tags : ["template", input.category]
  };
  ...
  await putFile(user.githubToken, campaign, path, serializePage(frontmatter, content), `CampaignRepo: create ${input.gameType} template ${input.name}`);
