...
import { commitFiles, listDirectoryTextFiles } from "@/lib/github";
...
const schema = z.object({
  ...
  set: z
    .object({
      ...
      // "" clears the parent; any other value sets it to that slug.
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // One GraphQL read of every page, then one commit for all the selected edits.
  const files = await listDirectoryTextFiles(user.githubToken, campaign, "wiki/pages");
  ...
  for (const file of files) {
    ...
    if (input.set.category) {
      fm.category = input.set.category;
      fm.type = input.set.category;
    }
    if (input.set.visibility) {
      fm.visibility = input.set.visibility;
      fm.knownToPlayers = input.set.visibility === "players";
    }
  ...
  if (updates.length) {
    await commitFiles(user.githubToken, campaign, updates, `CampaignRepo: bulk edit ${updates.length} pages`);
